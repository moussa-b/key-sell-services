...
exports.up = async function (knex) {
  const exists = await knex.schema.hasTable('sellers');
  if (!exists) {
    await knex.schema.createTable('sellers', (table) => {
      ...
      table
        .foreign('updated_by', 'fk_sellers_updated_by')
        .references('id')
        .inTable('users')
        .onDelete('SET NULL');
...
exports.down = async function (knex) {
  await knex.raw(`DROP TRIGGER IF EXISTS update_sellers_updated_at;`);
